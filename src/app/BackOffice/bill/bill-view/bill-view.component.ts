...
export class BillViewComponent implements OnInit {
   ...
   */
  payBill(): void {
    console.log('Starting payment process for bill:', this.bill);
    
    // Vérifier si la facture existe
    if (!this.bill) {
      this.errorMessage = "Impossible de procéder au paiement: facture non disponible";
      console.error('Attempted to pay bill but bill is null');
      return;
    }
    ...
    // Vérifier si l'ID de la facture est valide
    if (billId === undefined || billId === null) {
      this.errorMessage = "Impossible de procéder au paiement: ID de facture invalide";
      this.processingPayment = false;
      console.error('Bill ID is undefined or null');
      return;
    }
    
    console.log('Creating payment intent for bill ID:', billId);
  ...
  private openPaymentModal(clientSecret: string, paymentIntentId: string, billId: number): void {
    console.log('Opening payment modal with client secret:', clientSecret.substring(0, 10) + '...');
  ...
  /**
   * Détermine si le bouton de paiement doit être affiché
   */
  shouldShowPayButton(): boolean {
    if (!this.bill) {
      console.log('Payment button visibility check - Bill is null or undefined');
      return false;
    }
    
    console.log('Payment button visibility check - Payment mode:', this.bill.paymentMode);
    
    // Check if paymentMode is not 'PAID' (case insensitive)
    const paymentMode = this.bill.paymentMode?.toUpperCase() || '';
    const shouldShow = paymentMode !== 'PAID';
    console.log('Payment button should be shown:', shouldShow);
    
    return shouldShow;
  }

  /**
   * Formats the bill information for WhatsApp sharing
   * @returns URL-encoded string with bill details
   */
  encodeWhatsAppMessage(): string {
    if (!this.bill) return '';
    
    const message = `Facture #${this.bill.num_Bill}\n` +
      `Date: ${new Date(this.bill.date).toLocaleDateString()}\n` +
      `Montant: ${this.bill.total_Amount} EUR\n` +
      `Statut: ${this.getStatusDisplay(this.bill.status)}\n` +
      `Mode de paiement: ${this.getPaymentModeDisplay(this.bill.paymentMode)}`;
